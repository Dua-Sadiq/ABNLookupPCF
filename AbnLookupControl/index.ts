...
import { AbnComponent, IAbnProps } from "./AbnComponent";
...
export class AbnLookupControl implements ComponentFramework.StandardControl<IInputs, IOutputs> {
    ...
    // Mapping from state abbreviation to OptionSet value
    private stateOptions: Record<string, number> = {
        "NSW": 854500000,
        "VIC": 854500001,
        "QLD": 854500002,
        "WA": 854500003,
    ...
    public updateView(context: ComponentFramework.Context<IInputs>): void {
        ...
        this.stateValue = context.parameters.state.raw ?? 0;
    ...
    private onResultSelect = (result: CompanySearchResult) => {
        ...
        // Map state abbreviation to OptionSet value
        this.stateValue = this.stateOptions[result.State || ""] ?? 0;
    ...
    // Helper to get state abbreviation from OptionSet value
    private getStateAbbreviationFromValue(value: number): string {
        for (const [abbr, val] of Object.entries(this.stateOptions)) {
            if (val === value) return abbr;
        }
        return "";
    }
}
